...
class DDPG:
    ...
    def critic_loss(self, critic_params, state, batch, seed):
        ...
        # critic_loss = td_loss + 2 * jnp.logaddexp(-td_loss, 0)

        num_pos     = jnp.sum(td_loss >= 0)
        num_neg     = jnp.sum(td_loss <  0)
        neg_per_pos = num_neg / num_pos

        # p = 100.
        # critic_loss      = (td_loss ** p).sum(1) ** (1./p)
        ...
        aux = {
            ...
            'num_pos'        : num_pos.mean(),
            'num_neg'        : num_neg.mean(),
            'neg_per_pos'    : neg_per_pos.mean(),
