...
class MoG_DDPG(DDPG):
    ...
    def critic_loss(self, critic_params, state, batch, seed):
        ...
        next_action = self.actor_model_apply(state.target.actor, state.actor.sn_state, seed1, batch['next_obs'], self.args.critic_noise, .3, self.args.actor_dropout)[0]['action']
        target_info = self.critic_model_apply(state.target.critic, state.critic.sn_state, seed2, batch['next_obs'], next_action, self.args.target_dropout)[0]
